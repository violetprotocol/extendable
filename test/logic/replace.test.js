...
describe("ReplaceLogic", function () {
    ...
    describe("replace", () => {
        describe("simple replace", () => {
            ...
            it("replace should fail with non-existent extension", async function () {
            ...
            it("replace extend with different interface should fail", async function () {
                await replace(caller, 
                    SHOULD_REVERT, 
                    [extendLogic.address, mockNewExtendLogic.address],
                    [REPLACE_LOGIC_INTERFACE, PERMISSIONING_LOGIC_INTERFACE, EXTEND_LOGIC_INTERFACE], 
                    [replaceLogic.address, permissioningLogic.address, newExtendLogic.address]
                );
            });
...
const replace = async (caller, shouldRevert, [oldExtension, newExtension], expectedInterfaceIds, expectedContractAddresses) => {
    if (shouldRevert) await expect(caller.callReplace(oldExtension, newExtension)).to.be.reverted;
